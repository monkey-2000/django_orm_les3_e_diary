...
from datacenter.models import Lesson, Schoolkid, Commendation, \
    Mark, Chastisement
...
def get_schoolkid(schoolkid_name):
    try:
        ...
        message = f'Для имени "{schoolkid_name}" ' \
                  f'найдено несколько вариантов. Конкретизируйте запрос.'
        raise NameError(message)
    return child


def fix_marks(schoolkid, new_mark, bad_marks):
    Mark.objects.filter(
        schoolkid=schoolkid, points__in=bad_marks).update(points=new_mark)
...
def get_lesson_without_commendation(schoolkid, lessons):
    ...
    for parallel_lesson in parallel_lessons.order_by('-date'):
        if not child_commendation.filter(
                created=parallel_lesson.date).exists():
...
def create_commendation(schoolkid, lesson_name):
    ...
    if not lessons.exists():
        message = f'Предмет "{lesson_name}" не найден. ' \
                  f'Введите другое название предмета.'
        raise NameError(message)

    commendation_lesson = get_lesson_without_commendation(schoolkid, lessons)
...
def improve_school_results(
        schoolkid_name, lesson_name, new_mark=5,
        bad_marks=[2, 3]):
